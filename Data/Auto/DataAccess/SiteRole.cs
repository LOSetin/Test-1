...
namespace TMS
{
    ...
    {
        private const string TABLE_NAME = "SiteRole";
        private const string ALL_FIELDS = "ID,Name,MenuIDList,Remarks,ExtraFields";
        private const string SQL_ADD = "Insert Into SiteRole (Name,MenuIDList,Remarks,ExtraFields)Values(@Name,@MenuIDList,@Remarks,@ExtraFields)";
        private const string SQL_UPDATE = "Update SiteRole Set Name=@Name,MenuIDList=@MenuIDList,Remarks=@Remarks,ExtraFields=@ExtraFields Where ";
        private const string SQL_SELECT = "Select ID,Name,MenuIDList,Remarks,ExtraFields From SiteRole Where ";
        private const string SQL_DELETE = "Delete From SiteRole Where ";
        private const string SQL_TOP="Select Top {0} ID,Name,MenuIDList,Remarks,ExtraFields From SiteRole Where {1} Order By {2}";
        private const string SQL_COUNT="Select Count(*) From SiteRole Where ";
        ...
        private DbParameter[] makeParameterForUpdate(SiteRoleInfo model)
        {
            DbParameter[] paras = {
                    Db.Helper.MakeInParameter("@Name",model.Name),
                    Db.Helper.MakeInParameter("@MenuIDList",model.MenuIDList),
        ...
        public SiteRoleInfo Select(int iD)
        {
            string strSql = SQL_SELECT + PK_PARA_SET;
            DbParameter[] paras = { Db.Helper.MakeInParameter(PK_PARA, iD) };
            return Db.Helper.GetModel<SiteRoleInfo>(fillModel, strSql, paras);
        }

        public List<SiteRoleInfo> SelectList(string strWhere, DbParameter[] paras=null)
        {
            string strSql = SQL_SELECT + strWhere;
            return Db.Helper.GetList<SiteRoleInfo>(fillModel, strSql, paras);
        }

        public List<SiteRoleInfo> SelectList(int topNum, string strWhere, string strOrderBy, DbParameter[] paras=null)
        {
            string strSql = string.Format(SQL_TOP, topNum, strWhere, strOrderBy);
            return Db.Helper.GetList<SiteRoleInfo>(fillModel, strSql, paras);
        }

        public List<SiteRoleInfo> SelectList(string strWhere, string orderBy, int pageIndex, int pageSize, DbParameter[] paras=null)
        {
        ...
        #region  事务操作
        public int Add(SiteRoleInfo model, DbTransaction tran)
        {
            DbParameter[] para = makeParameterForAdd(model);
            return Db.Helper.ExecuteNonQuery(tran, SQL_ADD, para);
        }

        public int Update(SiteRoleInfo model, DbTransaction tran)
        {
            string strSql = SQL_UPDATE + PK_PARA_SET;
        ...
        public SiteRoleInfo Select(int iD, DbTransaction tran)
        {
            string strSql = SQL_SELECT + PK_PARA_SET;
            DbParameter[] paras = { Db.Helper.MakeInParameter(PK_PARA, iD) };
            return Db.Helper.GetModel<SiteRoleInfo>(fillModel, strSql, paras, true, tran);
        }

        public List<SiteRoleInfo> SelectList(string strWhere, DbTransaction tran, DbParameter[] paras=null)
        {
            string strSql = SQL_SELECT + strWhere;
            return Db.Helper.GetList<SiteRoleInfo>(fillModel, strSql, paras, true, tran);
        }
